function printpdf(fignum, figname)
% printpdf(fignum, figname)
...
%                                           %to test.eps.
% edited KIM 11.14 to fix terribleness of matlab script

% get the correct width of the figure
posi = get( fignum, 'position');
ww = posi(3)./72; % width figure in inches (apple default 72 dpi)
hh = posi(4)./72; % height of paper in inches
set(fignum, 'papersize', [ww, hh], 'paperposition', [0, 0, ww, hh], 'paperpositionmode', 'auto')

figfilestr = [figname '.pdf'];
eval(['print -dpdf -r150 -f' num2str(fignum) ' ' figname ';']);

% % hold over from printeps
% figfilestr = [figname '.eps'];
% eval(['print -depsc2 -r150 -f' num2str(fignum) ' ' figname ';']);
% 
